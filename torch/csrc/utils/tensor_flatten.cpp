...
std::vector<TensorGroup> take_tensors(
  ...
  for (const auto & tensor : tensors) {
    ...
    // tensor.options().id()
    auto& type_group = groups[id(tensor)];
...
void reorder_tensors_like(std::vector<Tensor>& tensors, TensorList order) {
  ...
  for (size_t i = 0, num_tensors = tensors.size(); i < num_tensors; ++i)
    type_id_to_indices[id(tensors[i])].push_back(i);
  ...
  for (auto & tmpl_tensor : order) {
    size_t type_id = id(tmpl_tensor);
    auto & indices = type_id_to_indices[type_id];
    auto & used = type_id_to_type_used[type_id];
